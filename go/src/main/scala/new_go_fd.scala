...
option("password", "R3@60n1Y$").load()
  ...
  */
  ...
  // val mrsDf2 = mrsDf1.unionAll(mrsSourceMain)
    ...
    res.write.mode("overwrite").format("com.databricks.spark.csv").option("delimiter", "\u0001").option("quoteMode", "NONE").option("escape", "\\").option("inferSchema","true").save("/antuit/databases/testwrite3/"+table);
